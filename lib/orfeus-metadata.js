    ...
    network.find("xmlns:Station", namespace).forEach(function(station) {
      ...
      channels.forEach(function(channel) {
        ...
        if(response.length === 0) {
          throw("Required response element is missing.");
        }

        if(response.length !== 1) {
          throw("Multiple response elements are included.");
        }
        ...
        if(1 - (Math.max(instrumentSensitivity, perStageGain) / Math.min(instrumentSensitivity, perStageGain)) > GAIN_TOLERNACE_PERCENT) {
          throw("The computed and reported total stage gain are different.");
        }
...
function setRestriction(node) {

  /* function setRestriction
   * Sets the nodes restrictedStatus attribute to closed 
   */

  node.attr("restrictedStatus", "closed");
...
function splitStationXML(files) {
  ...
  var restricted = files.filter(x => x.name === "restricted" && x.data === "on").length !== 0;

  for(var i = 0; i < files.length; i++) {

    // Skip anything that is not a submitted file
    ...
    if(!XMLDocument.validate(XSDSchema)) {
      throw("Error validating FDSNStationXML against schema.");
    }
    ...
    XMLDocument.find("xmlns:Network", namespace).forEach(function(network) {
      ...
      network.find("xmlns:Station", namespace).forEach(function(station) {
        ...
        if(restricted) {
          setRestriction(station);
          station.find("xmlns:Channel", namespace).forEach(setRestriction);
        }
