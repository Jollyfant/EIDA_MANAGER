...
function setChannelRestriction(station, restricted) {
...
function splitStationXML(files, properties) {
  ...
  files.forEach(function(file) {
    ...
    XMLDocument.get("xmlns:Network", namespace).forEach(function(network) {

      // The network must be set to restricted
      network.attr("restrictedStatus", getRestriction(properties.netRestricted));

      // Set the network description
      network.get("xmlns:Description", namespace).text(properties.description);

      // Set the end date to match that of the prototype
      setEndDate(network, properties.end)

      var networkCode = network.attr("code").value();
      var networkStart = convertDate(readAttribute(network, "startDate"));
      var networkEnd = convertDate(readAttribute(network, "endDate"));
      ...
      network.find("xmlns:Station", namespace).forEach(function(station) {
        ...
        setChannelRestriction(station, sRestricted);
