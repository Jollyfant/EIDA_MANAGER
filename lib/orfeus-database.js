...
Database.prototype.getAllUsers = function(callback) {
  ...
  this.users().find(new Object()).project(include).toArray(callback);
...
Database.prototype.addUser = function(postBody, callback) {
  ...
  if(postBody.username === "") {
    return callback(new Error("New user username field is empty"));
  }

  if(postBody.password === "") {
    return callback(new Error("New user password field is empty"));
  }

  if(postBody.role !== "admin" && postBody.role !== "user") {
    return callback(new Error("New user unknown role requested"));
  }
  ...
  this.getUserByName(postBody.username, function(error, document) {
    ...
    if(document !== null) {
      return callback(new Error("User with this name already exists"));
    }

    // Get the requested prototype from the identifier
    this.prototypes().findOne({"_id": this.ObjectId(postBody.prototype)}, function(error, document) {
      ...
      if(document === null) {
        return callback(new Error("Prototype could not be found"));
      }
