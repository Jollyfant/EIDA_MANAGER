...
function generateProfile(session) {
  ...
  if(session.role === "admin") {
    ...
    var fdsnwsQueryString = "?" + [
      "level=response",
      "network=" + session.network.code,
      (session.network.start ? "start=" + session.network.start.toISOString() : ""),
      (session.network.end ? "end=" + session.network.end.toISOString() : "")
    ].join("&");
  }
...
function generateWelcome(session) {
  ...
  return [
    "    <script>const USER_NETWORK = " + JSON.stringify(session.network) + ";</script>",
    "    <div class='container'>",
    "      <div style='text-align: center;'>",
    ...
    "        &nbsp;",
    "        <button class='btn btn-link' onclick='handleLogout()'><span class='fas fa-sign-out-alt' aria-hidden='true'></span><small><b> Logout</b></small></button>",
    "      </div>",
