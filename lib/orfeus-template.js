...
function generateProfile(session) {
  ...
  return [
    ...
    "        <li class='nav-item'>",
    "          <a class='nav-link active' role='tab' data-toggle='tab' href='#map-container-tab'><span class='far fa-map' aria-hidden='true'></span> &nbsp; Map Display</a>",
    "        </li>",
    "        <li class='nav-item'>",
    ...
    "        </li>",
    "        <li class='nav-item'>",
    "          <a class='nav-link' role='tab' data-toggle='tab' href='#settings-container-tab'><span class='far fa-file' aria-hidden='true'></span> &nbsp; Metadata</a>",
    "        </li>",
    "        <li class='nav-item'>",
    "          <a class='nav-link' role='tab' data-toggle='tab' href='#seedlink-container-tab'><span class='fas fa-seedling' aria-hidden='true'></span> &nbsp; Seedlink</a>",
    "        </li>",
    "        <li class='nav-item'>",
    ...
    "                    <input id='file-submit' class='btn btn-success' type='submit' value='Submit' disabled> &nbsp;",
    "                  </div>",
    "                </div>",
    "              </div>",
/*
...
*/
    ...
    "          <div style='display: none;' id='seedlink-submission'>",
    ...
    "              </div>",
    "              <div class='col-6 align-self-center text-muted' style='text-align: center;'>",
    "                <h1> Total of Data Exported",
    "                <br>",
    "                <b>273</b>GB </h1>",
    "              </div>",
    "            </div>",
    "          </div>",
    "          <hr>",
    "          <div id='citation-information' class='alert alert-light' style='text-align: center;'></div>",
    "        </div>",
    "      </div>",
    "    </div>",
    "  </body>",
...
function generateAdmin(session) {
  ...
  return [
    ...
    "        </li>",
    "        <li class='nav-item'>",
    "          <a class='nav-link' role='tab' data-toggle='tab' href='#prototype-tab'><span class='fas fa-globe-americas' aria-hidden='true'></span> &nbsp; Network Prototypes</a>",
    "        </li>",
    "        <li class='nav-item'>",
    "          <a class='nav-link' role='tab' data-toggle='tab' href='#rpc-tab'><span class='fas fa-cog' aria-hidden='true'></span> &nbsp; RPC</a>",
    "        </li>",
    "        <li class='nav-item'>",
    "          <a class='nav-link' role='tab' data-toggle='tab' href='#users-tab'><span class='fas fa-users' aria-hidden='true'></span> &nbsp; User Management</a>",
    "        </li>",
    "      </ul>",
    "      <div class='tab-content'>",
    ...
    "          </div>",
    "        </div>",
    "        <div class='tab-pane' id='prototype-tab' role='tabpanel'>",
    ...
    "              </div>",
    "              <input minlength='3' name='username' class='form-control' placeholder='Username' required autofocus>",
    "              <div class='input-group-prepend'>",
    ...
    "              <input type='password' minlength='6' name='repassword' class='form-control' placeholder='Re-enter' required>",
    "            </div>",
    "            <br>",
    "            <div class='input-group mb-3'>",
    "              <div class='input-group-prepend'>",
    "                <label class='input-group-text' for='prototype-select'><span class='fas fa-globe-americas' aria-hidden='true'></span>&nbsp; Network Prototype</label>",
    "              </div>",
    "              <select id='prototype-select' class='custom-select' name='prototype'></select>",
    "            </div>",
    "            <div class='input-group mb-3'>",
    "              <div class='input-group-prepend'>",
    "                <label class='input-group-text' for='role-select'><span class='fas fa-user-tag' aria-hidden='true'></span>&nbsp; Role</label>",
    "              </div>",
...
function generateWelcome(session) {
  ...
  var clientConfig = {
    ...
    "NODE": CONFIG.NODE,
    "FDSNWS": CONFIG.FDSNWS,
    "MODULES": CONFIG.MODULES,
    "NETWORK": session.prototype,
    "ROLE": session.role,
    "ADMINISTRATOR": session.role === database.ROLES.ADMINISTRATOR
  }

  return [
    "    <script>const USER_NETWORK = " + JSON.stringify(session.prototype) + ";const CONFIG = " + JSON.stringify(clientConfig) + ";</script>",
    "    <div class='container'>",
    "      <div style='text-align: center;'>",
    getLogo(CONFIG.NODE.ID),
    "      </div>",
    "      <div style='float: right;'>",
...
function generateStationDetails(session) {
  ...
  return [
    ...
    "              <button type='button' class='close' onclick='toggleElement(\'response-charts\', false)' aria-label='Close'>",
...
function generateNewMessageTemplate(invalid, session) {
  ...
  function getMessageTip(role) {

    /*
     * Function generateNewMessageTemplate:getMessageTip
     * Returns HTML for tip when sending a new private message
     */

    if(role === database.ROLES.ADMINISTRATOR) {
      return "To broadcast a message to all users specify <b>broadcast</b> as the recipient.";
    } else {
      return "To send a message to the administrators specify <b>administrators</b> as the recipient.";
    }
  }
  ...
  return [
    ...
    "        <div id='message-information' style='text-align: center;'></div>",
    ...
    "      <small>Tip! " + getMessageTip(session.role) + "</small>",
    "      </form>",
    generateFooter(),
    generateFooterApp()
  ].join("\n");
...
function generateMessages(session) {
  ...
  return [
    ...
    "    <ul class='nav nav-tabs nav-justified' role='tablist'>",
    "      <li class='nav-item'>",
    "        <a class='nav-link active' role='tab' data-toggle='tab' href='#messages-inbox-tab'><span class='fas fa-inbox' aria-hidden='true'></span> &nbsp; Message Inbox</a>",
    "      </li>",
    "      <li class='nav-item'>",
...
function generateFooter() {
  ...
  return [
    ...
    "  <hr>",
    "  <b><span class='text-danger'>E</span>IDA Manager</b> &copy; " + new Date().getFullYear() + " <a href='https://orfeus-eu.org'>ORFEUS Data Center</a>. All Rights Reserved.",
    "  <div style='float: right;' class='text-muted'><small>Version v" + CONFIG.__VERSION__ + "</small></div>",
    "  </footer>",
...
function getLogo(id) {
  ...
  /*
   * Function getLogo 
   * Points to path for institution logo and styling
   */

  switch(id) {
    case "ODC":
      return "<img src='/images/knmi.png' style='height: 100px;'>";
    case "GFZ":
      return "<img src='/images/gfz.png' style='height: 100px; margin-top: 20px;'>";
    case "NOA":
      return "<img src='/images/noa.png' style='height: 100px; margin-top: 20px;'>";
    case "SED":
      return "<img src='/images/sed.png' style='height: 100px; margin-top: 10px;'>";
    case "NIEP":
      return "<img src='/images/niep.png' style='height: 100px;'>";
    case "RESIF":
      return "<img src='/images/resif.png' style='height: 100px;'>";
    case "BGR":
      return "<img src='/images/bgr.png' style='height: 100px;'>";
    case "INGV":
      return "<img src='/images/ingv.png' style='height: 100px; margin-top: 20px;'>";
    case "KOERI":
      return "<img src='/images/koeri.png' style='height: 100px; margin-top: 10px;'>";
    case "LMU":
      return "<img src='/images/lmu.png' style='height: 100px;'>";
    default:
      return "";
  }
...
function generateHTTPError(statusCode, error) {
  ...
  function getErrorMessage(statusCode) {
    ...
    switch(statusCode) {
      ...
      case 418:
        return "I'm a teapot.";
      case 500:
        return "The server experienced an unrecoverable internal error.";
      case 501:
  ...
  return [
    ...
    error ? "<br><span class='text-muted'><small><b>Details</b>: " + (CONFIG.__DEBUG__ ? error.stack : error.message) + "</small></span>" : "",
    "    </div>",
    "  </body>",
    generateFooter(),
    "</html>"
