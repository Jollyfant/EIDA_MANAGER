...
const { E_HTTP_TEAPOT } = require("./orfeus-http");
const CONFIG = require("../config");

generateProfile = function(session) {
  ...
  return [
    ...
    "              &nbsp;&nbsp;<span class='text-muted'><span class='fa fa-clock-o'></span><b> Pending</b></span> - Metadata was submitted",
    ...
    "          <small>Your EIDA Data Center (" + CONFIG.NODE.ID + ") connects with the following IP address: <b>" + CONFIG.EXTERNAL.IP + ".</b></small>",
    "        </div>",
    "      </div>",
    "    </div>",
    "  </body>",
...
generateWelcomeInformation = function(session) {
...
generateWelcome = function(session) {

  return [
    "    <script>const USER_NETWORK = '" + session.network + "'; USER_VERSION = " + JSON.stringify(session.version) + "</script>",
    "    <div class='container'>",
    "      <div style='text-align: center;'>",
...
generateStationDetails = function(session) {
...
generateNewMessageTemplate = function(invalid, session) {
  ...
  return [
    ...
    "      <small>Tip! To send a message to all administrators specify <b>administrators</b> as the recipient</small>",
    "      </form>",
    generateFooter(),
    generateFooterApp()
  ].join("\n");
...
generateMessageDetails = function(session) {
...
generateFooter = function() {
  ...
  return [
    ...
    "  <hr>",
    "  <b><span class='text-danger'>E</span>IDA Manager</b> &copy; " + new Date().getFullYear() + " <a href='https://orfeus-eu.org'><b>ORFEUS Data Center</b></a>. All Rights Reserved. Licensed under <a href='https://opensource.org/licenses/MIT/'><span class='fa fa-balance-scale'></span><b> MIT</b></a>.",
...
generateLogin = function(invalid) {
...
generateInvalid = function(invalid) {
 ...
 *       */
  ...
  if(invalid.endsWith("invalid")) {
    return generateMessageAlert("danger", E_CREDENTIALS_INVALID);
  }
...
function generateHTTPError(status) {
   ...
   * Returns HTTP for invalid status
   */
  ...
  return [
    ...
    "    <div class='container'>",
    "      <h2 class='text-muted'><span style='color: #C03;'>" + status +"</span> " + STATUS_CODES[status] + " </h2>",
    "    </div>",
    "  </body>",
    generateFooter(),
    "</html>"
