...
function metadMerge(document) {
  ...
  database.prototypes().find({"network": document.network}).sort({"created": database.DESCENDING}).limit(1).toArray(function(error, prototypes) {
...
function metadCheck(document) {
  ...
  logger.info("metadCheck is requested for " + document.network.code + "." + document.station);

  // Request instrument metadata
  var query = [
    "network=" + document.network.code,
    "starttime=" + document.network.start.toISOString(),
    "station=" + document.station,
    "level=response"
  ];

  if(document.network.end !== null) {
    query.push("endtime=" + document.network.end.toISOString());
  }

  // Make a HTTP request to the webservice
  ohttp.request(CONFIG.FDSNWS.STATION.HOST + "?" + query.join("&"), function(data) {
    ...
    if(SHA256(network.toString()) === document.sha256) {
      metaDaemonCallback(document, database.METADATA_STATUS_COMPLETED);
    } else {
      metaDaemonCallback(document, database.METADATA_STATUS_UNCHANGED);
    }
