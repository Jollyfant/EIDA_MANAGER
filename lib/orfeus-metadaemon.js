...
function metadFull(callback) {
  ...
  Console.info("metaDaemon is merging the entire inventory.");
  ...
  Database.files().aggregate(pipeline).toArray(function(error, documents) {
    ...
    Console.info("metaDaemon is merging " + documents.length + " inventory files."); 
    ...
    // Child process has closed
    convertor.on("close", function(code) {
      Console.info("metaDaemon merged full inventory. Exited with status code " + code + ".");
      callback();
    });
