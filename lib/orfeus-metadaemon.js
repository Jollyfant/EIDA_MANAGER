...
(metaDaemonInit = function() {
  ...
  database.files().aggregate(pipeline).toArray(function(error, results) {
    ...
    (GLOBAL_CALLBACK = function() {
      ...
      // Asynchronously set previous documents to superseded
      supersedePrevious(document);
...
function supersedePrevious(document) {

  /* Function supersedePrevious
   * Set previous documents to superseded
   */

  logger.info("Setting previous files for " + document._id.network + "." + document._id.station + " to superseded");

  database.files().updateMany({
    "network": document._id.network,
    "station": document._id.station,
    "_id": {
      "$ne": database.ObjectId(document.id)
    }
  }, {
    "$set": {
